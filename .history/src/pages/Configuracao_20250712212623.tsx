...
import { Licao } from '../database/supabaseService';
...
const Configuracao: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMensagem('Erro ao salvar a lição: ' + (err?.message || JSON.stringify(err)));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-200">
      <button
        onClick={() => navigate('/')}
        className="self-start ml-4 mb-4 bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-800 transition"
      >
